...
As CUB is developed both on GitHub and internally at NVIDIA, there's three main places where code lives:
...
The version number for a CUB release uses the following format: `MMM.mmm.ss-ppp`, where:
   ...
   * `CUB_VERSION_MAJOR`/`MMM`: Major version, up to 3 decimal digits. It is incremented
     ...
     versions.
   * `CUB_VERISON_MINOR`/`mmm`: Minor version, up to 3 decimal digits. It is incremented when
